...
import { Plus, Edit2, Trash2, X, RefreshCw, Upload, AlertCircle, Boxes } from 'lucide-react';
...
const AdminCombos = () => {
  ...
  const [editComboId, setEditComboId] = useState(null); // If editing, holds the combo ID, else null
  ...
  const fetchCombosData = async () => {
    setLoading(true);
    try {
      const comboRes = await API.get('/products/combos');
      setCombos(comboRes.data);
    } catch (err) {
      console.error('Fetch combos admin failed:', err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveCombo = async (e) => {
    ...
    try {
      if (editComboId) {
        // Edit route
        await API.put(`/admin/combos/${editComboId}`, payload);
      } else {
        // Create route
        await API.post('/admin/combos', payload);
      }
      setIsModalOpen(false);
      fetchCombosData(); // Reload list
    } catch (err) {
      setFormError(err.response?.data?.message || 'Error saving combo details.');
    } finally {
  ...
  const handleDeleteCombo = async (id) => {
    if (!window.confirm('Are you absolutely sure you want to delete this combo? Products inside this combo pack will remain in database but the combo relation will be removed.')) {
      return;
    }
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Error deleting combo.');
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-brand-dark dark:text-white uppercase tracking-tight font-heading flex items-center gap-2">
            <Boxes className="text-brand-blue" size={24} /> Combos Catalog CRUD
          </h1>
          <p className="text-xs font-semibold text-brand-grey dark:text-zinc-400 uppercase tracking-wider font-heading mt-0.5">
            Add, update, or remove formulation combo kits from database
          </p>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 space-y-3">
          ...
          <p className="text-xs font-semibold text-brand-grey dark:text-zinc-400 uppercase tracking-wider">
            Loading combos database...
          </p>
        </div>
      ) : combos.length === 0 ? (
        <div className="text-center p-12 bg-white dark:bg-zinc-900 border border-brand-border dark:border-zinc-800 rounded-xl">
          <p className="text-xs text-brand-grey dark:text-zinc-400 font-medium">No combos found in the database. Click Add Combo to get started.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left text-xs text-brand-dark dark:text-zinc-200">
              <thead>
                <tr className="border-b border-brand-border dark:border-zinc-800 bg-brand-bg-grey dark:bg-zinc-850 text-brand-grey dark:text-zinc-450 font-bold uppercase tracking-wider">
                  ...
                  <th className="py-4 px-4">Combo Name</th>
              ...
              <tbody className="divide-y divide-brand-border/60 dark:divide-zinc-800/60 font-medium">
                {combos.map((combo) => (
                  <tr key={combo.id} className="hover:bg-brand-bg-grey/30 dark:hover:bg-zinc-800/30">
                    <td className="py-3 px-4">
                      <div className="w-10 h-10 bg-brand-bg-grey dark:bg-zinc-800 border border-brand-border dark:border-zinc-700 rounded overflow-hidden flex items-center justify-center">
                        <img 
                          ...
                          onError={(e) => { e.target.src = '/assets/placeholder-combo.jpg'; }}
                        />
                      </div>
                    </td>
                    <td className="py-3 px-4 font-bold text-brand-dark dark:text-white">{combo.name}</td>
                    <td className="py-3 px-4 font-mono text-brand-grey dark:text-zinc-450">{combo.slug}</td>
                    <td className="py-3 px-4 text-brand-grey dark:text-zinc-400 max-w-xs truncate" title={combo.description}>{combo.description}</td>
          ...
          <div className="bg-white dark:bg-zinc-900 border border-brand-border dark:border-zinc-800 rounded-xl w-full max-w-md flex flex-col shadow-2xl animate-fade-up">
            ...
            <div className="flex justify-between items-center px-6 py-4 border-b border-brand-border dark:border-zinc-800">
              <h2 className="text-sm sm:text-base font-bold text-brand-dark dark:text-white font-heading uppercase tracking-wide">
                {editComboId ? 'Edit Combo' : 'Create New Combo'}
              </h2>
            ...
            <form onSubmit={handleSaveCombo} className="p-6 space-y-4">
              ...
              {/* Combo Name */}
              <div>
                <label className="block text-[10px] font-bold text-brand-grey dark:text-zinc-400 uppercase tracking-wider mb-1">Combo Name *</label>
                <input
                  type="text"
                  required
                  value={name}
                  ...
                  placeholder="e.g. Anti-Acne Essential Kit"
                />
              </div>

              {/* Image upload */}
              <div className="space-y-1">
                <span className="block text-[10px] font-bold text-brand-grey dark:text-zinc-400 uppercase tracking-wider mb-1">Combo Image</span>
                <div className="grid grid-cols-3 gap-3 items-center">
                  <input
                    ...
                    placeholder="/assets/placeholder-combo.jpg"
                  />
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-brand-border dark:border-zinc-700 bg-white dark:bg-zinc-800 text-brand-dark dark:text-white rounded text-xs focus:outline-none focus:border-brand-blue"
                  placeholder="Formulation kit for curing active skin breakouts..."
                ></textarea>
              </div>
              ...
              <div className="border-t border-brand-border dark:border-zinc-800 pt-4 flex justify-end gap-3 bg-white dark:bg-zinc-900 sticky bottom-0">
                ...
                >
                  {formSubmitting ? 'Saving...' : 'Save Combo'}
                </button>
              </div>
            </form>
